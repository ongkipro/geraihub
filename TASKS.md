...
> **Execution status: Development authorized; T-1 and T-3 locally verified; T-4 is next.** This is the sole canonical implementation queue. Source-code authorization does not authorize secrets, provider mutations, production resources, deployment, or release.
...
- Status: Complete — committed clean-checkout foundation verification passed
- Owner: Engineering owner
- Primary requirement: TD-9
- Constraints: SEC-8, SEC-15, PRIV-13
- Dependencies: T-2, T-23
- Current evidence: Commit `402b441` on `feat/development-foundation` pins Node 24/pnpm 11.22.0, Next.js 16.3.6, React 19.3.0, PostgreSQL 17 local probe, Drizzle 0.45.3/Kit 0.31.11, and Better Auth 1.7.5. The web/worker source, lockfile, reviewed probe SQL, seed, and two foundation tests exist. Clean-checkout proof is recorded below; hosted CI for this commit remains unobserved.
...
- Local result (2026-09-24, Node 24.18.0, pnpm 11.22.0, PostgreSQL 17 Alpine, synthetic data): From an isolated source export without `node_modules`, build output, or generated Next types, frozen install PASS; migration generation found no drift; migration and synthetic seed PASS against a new disposable database; lint and typecheck PASS; 2/2 Node tests PASS; Next production build PASS; production web startup returned HTTP 200; worker startup reported ready with provider dispatch disabled. The existing worktree's development web startup also returned HTTP 200, and Chromium displayed the foundation route at desktop and narrow widths. A negative ESLint probe rejected a worker import of `src/app/`.
- Clean-checkout result (2026-09-24): Detached Git worktree at `402b441`, initially clean and without dependencies/build artifacts, used a fresh disposable PostgreSQL 17 Alpine database on local loopback. `pnpm install --frozen-lockfile`, `pnpm db:generate` plus `git diff --exit-code -- drizzle`, `pnpm db:migrate`, `pnpm db:seed`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (2/2), and `pnpm build` all PASS. Production `pnpm start` served HTTP 200 on `/`; `pnpm worker` reported ready with provider dispatch disabled. `python3 scripts/check-repository.py .` PASS (37 required files, 41 Markdown files), validator regressions PASS (18/18), `actionlint` PASS. The checkout remained clean after verification. This closes the T-1 local acceptance; hosted GitHub Actions and all later domain/security/provider behavior remain separate evidence.
...
- Status: Complete — three persistence units locally verified; runtime OAuth and authorization remain T-4/T-5
- Owner: Engineering owner
...
- Unit 1 boundary (2026-09-24): Add only `organizations` and `branches` with opaque IDs, required branch organization ownership, constrained branch lifecycle, timezone identity and version. Generate and review additive SQL, then run `DATABASE_URL=<disposable-local-db> pnpm db:migrate` and `DATABASE_URL=<same-db> node --experimental-strip-types --test tests/organization-branch.test.ts`; the test must prove valid two-organization persistence, missing-parent/invalid-state denial, and immutable organization/branch identity including no deletion/reuse. No route or branch activation authority is granted by these rows; T-14 owns the authorized lifecycle transitions.
- Unit 1 result (2026-09-24): Reviewed `drizzle/0001_classy_shinko_yamashiro.sql`: two additive tables, constraints, restrictive organization FK, and explicit identity/lifecycle/version triggers; no drop, rename, cast, backfill, or existing-row rewrite. Migration PASS both upgrading a disposable PostgreSQL 17 database with T-1 already applied and from a second empty disposable PostgreSQL 17 database. `tests/organization-branch.test.ts` PASS against both targets; it verifies two organization keys, branch ownership, missing parent, invalid status/timezone shape, blocked reparent/ID update/delete, version increment/stale update, and terminal retirement. Full `pnpm test` PASS 3/3; `pnpm lint`, `pnpm typecheck`, `pnpm build`, repository validator and `git diff --check` PASS. One Drizzle Kit 0.31.11 generation failure on a JavaScript `1n` default was resolved by a SQL literal default of `1` and regeneration before migration. This proves database constraints only; no membership authorization or activation workflow is implemented.
- Unit 2 boundary (2026-09-24): Generate the installed-version Better Auth PostgreSQL/Drizzle physical schema with UUID user/session/account IDs and review the SQL before applying it. Use the auth `user` row as GeraiHub's stable internal identity; add a separate server-owned active-branch context linked to the auth session and constrained to a branch/organization pair. Keep OAuth/Google callback, invitation acceptance, and actual branch-switch authorization in T-4/T-5. Verify migration on a disposable local PostgreSQL database and run `DATABASE_URL=<disposable-local-db> node --experimental-strip-types --test tests/identity-context.test.ts` with malformed/cross-scope/duplicate-context cases. No live credential or OAuth request is involved.
- Unit 2 result (2026-09-24): Better Auth 1.7.5's pinned `auth` CLI generated the PostgreSQL/Drizzle schema from `scripts/auth-schema.config.ts`; the reviewed schema adds timezone-aware timestamps, unique `(provider, issuer, subject)`, a Google-only issuer constraint, and a database prohibition on OAuth/password token persistence. `drizzle/0002_illegal_imperial_guard.sql` is additive and adds session/context composite FKs plus immutable account/session-user guards. A final fresh disposable PostgreSQL 17 migration and synthetic seed PASS; `tests/identity-context.test.ts` and `tests/auth-adapter.test.ts` PASS, including actual installed Drizzle adapter create/read. Full `pnpm test` PASS 4/4, lint/typecheck/peer check and repository validator PASS. The earlier disposable database received an intermediate migration draft; no shared or production database was touched. Runtime OAuth hooks must strip provider tokens before database writes and enforce invitation authorization under T-4; these physical tests do not prove login or branch authorization.
- Unit 3 boundary (2026-09-24): Add organization/branch-scoped memberships, separate platform grants, expiring/revocable one-time invitations with a non-secret proof digest, and a permanent singleton bootstrap marker. Review the additive migration, then implement one atomic invitation-consumption function fed only a trusted verified identity. Run `DATABASE_URL=<disposable-local-db> node --experimental-strip-types --test tests/membership-invitation.test.ts` for correct grants, wrong email/subject, expiry/revocation/replay/concurrency, cross-organization branch denial, duplicate active grant rejection, and bootstrap-marker irreversibility. The Google callback and real operator bootstrap command remain T-4.
- Unit 3 result (2026-09-24): Reviewed additive `drizzle/0003_flowery_redwing.sql`, including four tables, composite branch ownership FKs, unique active role indexes, immutable invitation/grant history triggers, and a one-row permanent bootstrap marker. A failed platform-grant revocation test exposed a shared trigger referencing membership-only columns; the trigger was fixed and all migrations rerun from an empty disposable PostgreSQL 17 database. `tests/membership-invitation.test.ts` verifies two-branch memberships, cross-organization denial, duplicate active role denial, grant revocation, one-time concurrent acceptance (one success/one denial), wrong email/subject, expiry, revocation, and bootstrap completion irreversibility. Full `pnpm test` PASS 8/8; `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm db:generate` (no drift), peer check, repository validator, and `git diff --check` PASS. T-3 persistence is complete locally; the `acceptInvitation` input is trusted only after T-4 verifies Google and binds the callback, and no HTTP route exposes it today. No live OAuth, real bootstrap, or active-branch authorization is claimed.

### T-4 — Implement Google OAuth and GeraiHub provisioning
- Status: In progress — controlled synthetic bootstrap unit started; OAuth and recovery remain open
- Owner: Engineering owner
- Primary requirement: PR-11
...
- Unit 1 boundary (2026-09-24): Implement a non-HTTP, environment-scoped operator bootstrap command using a dedicated operator connection, explicit approval reference/operator/environment/intended email, and a protected output artifact for the one-time proof. The transaction locks the permanent marker, creates at most one live invitation, atomically revokes an expired one before replacement, and denies replay after completion. Add database privilege rules preventing the runtime role from issuing bootstrap invitations or resetting the marker. Run `DATABASE_URL=<disposable-local-db> node --experimental-strip-types --test tests/bootstrap-operator.test.ts` for concurrent issue, expiry replacement, completion replay, and runtime-role denial. No real identity, proof, or external OAuth client is used.
- Unit 1 local result (2026-09-24): A fresh synthetic PostgreSQL 17 database applied migrations `0001`–`0005` and seed. The operator command keeps the proof in a `0600` file outside the repository; the first invitation binds the environment marker, concurrent replacement of an expired invitation has one winner, completed bootstrap cannot replay, and a runtime role cannot issue a bootstrap invitation or retarget the marker. A review found newly added approval metadata missing from the invitation immutability trigger; migration `0005` now protects it and the test rejects mutation. `pnpm test` passed 10/10, with lint, typecheck, build, schema generation without drift, repository validator, and diff check passing locally. This is a partial T-4 result: OAuth callback/verified identity, runtime-role invitation acceptance, operator audit/recovery, browser scenario 11, and real credential setup remain open; no live administrator was provisioned.
