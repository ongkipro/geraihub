...
- Change history: Added during planning audit to anchor setup and stack tasks; stack selection remains unapproved.
...
- Internal operation boundaries and contract gates are defined in `09-API-SPECIFICATION.md`; implementation selects the exact HTTP/session/schema representation from installed framework evidence. Do not invent provider cancellation endpoint or payload until current Mengantar contract is verified.
...
The repository exists at `~/Projects/geraihub/` as a documentation-only planning repository. It has no source tree, package manifest, lockfile, installed dependency, or runtime evidence. Modular monolith, PostgreSQL, Drizzle, Better Auth, and a web framework remain target recommendations, not accepted implementation evidence. T-2 selects the compatible runtime/deployment profile from current official documentation; T-1 may initialize and verify locked package versions only after explicit development authorization. Do not treat planning-machine tools as project evidence.
...
- Automated transition, idempotency, tenant/branch authorization, and provider-adapter contract tests.
