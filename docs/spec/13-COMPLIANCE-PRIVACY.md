...
GeraiHub processes sender and recipient names, telephone numbers, addresses, optional branch-local reusable contact entries, pickup-point configuration, shipment item descriptions/quantities/declared values, package/service/COD details, operator identity, branch membership, invoice charge/reference, and provider resi/status so an authorized gerai can create and manage a shipment through Mengantar. It is not an online checkout, payment processor, wallet, COD settlement system, or customer-facing pre-fill service in MVP.
...
|---|---|---|---|---|---|---|
| PRIV-1 | Privacy owner | Sender/recipient name, phone, address and explicitly saved branch-local contact entry | Shipment customer/sender/recipient | Create, verify, submit, print, and optionally reuse a contact for a later same-gerai shipment | Authorized branch users, GeraiHub database/backup, Mengantar for shipment data | Collect only fields required by verified provider contract; scope/mask directory search, avoid automatic contact creation/merge, redact logs/audit; no public MVP lookup. |
| PRIV-2 | Privacy owner | Pickup point, item descriptions/quantities/declared values, package, service, COD, quote, resi, provider status | Customer/shipment | Shipment operations, reconciliation, operational reporting | Authorized branch users, Mengantar only where verified contract requires, database/backup | Scoped read/write; minimize item free text and invoice display; no payment credentials; resi not used as public identifier. |
...
| PRIV-4 | Privacy owner | Issued invoice reference, branch, charge, quote/resi link, issue time and issuance/reprint audit | Customer/shipment and operator | Document the confirmed shipment charge and preserve history | Authorized branch/finance roles, database/audit | Limit customer details to approved invoice purpose; no customer payment method, status, receipt, correction, or refund data. |
