...
| CTX-2 | Operating entities | Organization, gerai branch and its pickup points, owner/admin/staff, branch-local reusable sender/recipient contacts, shipment items, shipment, and Mengantar are the operational frame. | Decision | human-policy | 02-PRD.md section 2; 05-DATA-MODEL.md | GeraiHub product owner | Ownership or workflow changes |
...
| CTX-6 | Commerce | Non-COD is the default; COD ongkir and COD produk with optional included shipping are gated courier-collection modes. GeraiHub estimates a separate 3.33% fee on intended COD amount; actual provider fee/rounding/payer remain unverified. Provider finance remains separate from manual customer payment handled by the gerai. GeraiHub issues invoices and confirmed resi, but does not record gerai payment or operate a wallet, gateway, or settlement ledger. | Decision | human-policy | Q-3, Q-8, Q-9; BILL-1 through BILL-8 | Finance owner | Tender, COD fee, refund, or settlement role changes |
...
| CTX-12 | Deferred pre-fill | Post-MVP customer pre-fill presents a short non-sequential readable reference and QR for the same draft, distinct from Mengantar resi; counter verification remains required; manual customer payment stays outside the app. | Decision | human-policy | PR-12; 17-UX-FLOWS-SCREEN-CONTRACTS.md section 6 | GeraiHub product owner | Separate post-MVP authorization |
...
|---|---|---|---|---|---|---|---|
| OVR-1 | commerce | Active | CTX-6 | commerce, privacy, security | Finance owner | Invoice and non-authoritative provider finance | Invoice replacement policy and exact formula review |
...
| Billing | Selected | Finance owner | Immutable customer invoices, manual-payment boundary, and non-authoritative provider finance; post-issue replacement policy remains open |
...
On 2026-09-23, T-23 branch protection was enabled and read back through GitHub. The product owner subsequently confirmed that customer payment is entirely manual outside GeraiHub. The application issues a charge invoice and provider-confirmed resi; the earlier QR payment and payment-record assumptions are superseded. The task traceability matrix now lists only actual primary tasks. These changes do not create application runtime evidence.
...
Material corrections: formal requirement ownership; one jurisdiction register and preserved supersession; explicit IAM/TEN/API/UX ownership; bullet task metadata; dedicated invoice and profit-report tasks; removal of staff user-management/admin self-promotion contradictions; accepted invoice/manual-payment, expiry, and auth policies distinguished from provider/operational gates; provenance dates no longer claimed verified. Core product requirements remain unchanged. Owner counter permissions remain explicit-role only; JIT support is read-only with distinct approval; invoice replacement remains gated for MVP. Actual staffing/approvers and runtime enforcement still require evidence before release.
...
- [x] Account recovery/invitation identity policy, JIT approval policy, manual-payment boundary and invoice policy recorded in canonical specs; post-issue invoice replacement remains gated.
