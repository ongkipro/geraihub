...
| CTX-5 | Market and locale | Indonesia, Bahasa Indonesia, and IDR are the current scope; Asia/Jakarta as a universal business-time default is still an assumption requiring branch policy. | Assumption | hypothesis | 02-PRD.md scope; 10-DESIGN-SYSTEM-WHITELABEL.md locale and open decisions | GeraiHub product owner | Branch geography, business calendar, or locale changes |
...
| CTX-11 | Locale contract | id-ID UI, IDR presentation, and explicit timezone display are the proposed localization contract; WCAG 2.2 AA is an engineering target, not certification. | Proposal | inference | LOC-1; UI-2, UI-4; design-system sections 3–5 | Design owner | Locale/accessibility requirements change |
...
| OVR-3 | identity | Active | CTX-7, CTX-17 | iam, security | Security owner | Google identity with GeraiHub authorization | IAM-1 and auth/recovery policy review |
...
|---|---|---|---|---|---|---|---|
| LOC-1 | id-ID | en for technical diagnostics only, never a substitute for required Indonesian operator copy | Exact IDR formatting and explicit branch timezone; business calendar/cutoff remains a policy gate | WCAG 2.2 AA engineering target; desktop/tablet counter flow, narrow-screen recovery and zoom reflow | TEST-1 (planned procedure, not an observed pass) | Design owner | Proposed |
